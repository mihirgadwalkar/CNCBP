...
{
    document.getElementById("para").innerHTML += " <br><br> The Sender has access to this channel now!. <br><br> Message transmission from  " + list_devices[document.getElementById("device1").value].mac_address + " to  " + list_devices[document.getElementById("device2").value].mac_address + " <br><br> Sending Packets...";
}
...
function end_to_end()
{
    ...
    else if(check == "No Port Available"){
...
function bridge_to_end()
{
    ...
    {
        alert("Sorry! You Entered a Wrong Segment!");
    }
}
...
function hub_to_switch()
{
    ...
    else if(check1 == "No Port Available"){
        alert("Ports are full");
    }
    else{
        alert("Successfully Connected");
...
function R_end_to_end()
{
    ...
    else if(check == "No Connection exists!"){
...
export  function stop_wait(){
    for(var i = 0; i < arguments[2].length; i++){
        ...
        document.getElementById("para").innerHTML += " <br><br> Sequence Number is " + x + ". Sending packet " + i + " ....";
        if(ranDom >= 0.3){
...
export  function selective_rep(){
    ...
    while(n){
        var check = 0;
        for(sn = sf; sn < arguments[2].length && sn <= sf + window_size-1; sn++){
            if(sender_window[sn] == "-1"){
                ...
                if(ranDom >= 0.3){
                    ...
                    document.getElementById("para").innerHTML += " <br><br> Packet "+ (sn) + " with Sequence Number : " + sn % 8 + " sent successfully ....Received ACK " + (sn % 8) + " from the receiver";
                    n--;
                    if(sn == sf){
                    ...
                    document.getElementById("para").innerHTML += " <br><br> Packet " +(sn)+ " with Sequence Number : " + sn % 8 + " failed. No ACK received";
                    if(!check){
                        sf = sn;
